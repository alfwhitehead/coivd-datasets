...
cd $dir
date > run.log
/usr/local/bin/wget --no-check-certificate 'https://docs.google.com/uc?export=download&id=1euhrML0rkV_hHF1thiA0G5vSSeZCqxHY' -O $today.csv  >run.log 2>&1
git add $today.csv >run.log 2>&1
